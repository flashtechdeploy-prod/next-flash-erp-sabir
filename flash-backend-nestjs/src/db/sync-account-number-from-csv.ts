...
async function syncAccountNumberFromCsv() {
  try {
    console.log('Starting account_number sync from CSV...');
    ...
    for (const record of records) {
      ...
      // Only process if CSV has blank account_number
      ...
      for (const match of matches as EmployeeMatch[]) {
        // If DB has account_number but CSV is blank, clear it
        if (!isBlank(match.account_number)) {
          if (!DRY_RUN) {
            await db
          ...
          console.log(
            `${prefix}✓ Cleared FSS ${match.fss_no ?? fssRaw} account_number (was: ${match.account_number})`
          );
        } else {
          alreadyClear++;
        }
    ...
    console.log(`Account_number Cleared: ${cleared}`);
